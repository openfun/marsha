...
describe('useConfig', () => {
  ...
  it('successfully calls the API', async () => {
    fetchMock.getOnce('/api/config/?domain=localhost', {
      sentry_dsn: 'https://sentry.io/1234567',
      environment: 'test',
      release: '1.0.0',
      inactive_content_types: ['test'],
    });
    ...
    expect(fetchMock.lastCall()![0]).toEqual(`/api/config/?domain=localhost`);
  ...
  it('manages failure', async () => {
    fetchMock.getOnce('/api/config/?domain=localhost', 401);
