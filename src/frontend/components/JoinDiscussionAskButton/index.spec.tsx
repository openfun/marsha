...
describe('<JoinDiscussionAskButton />', () => {
  ...
  it('renders the ask button', () => {
    ...
    expect(
      screen.queryByText('Waiting for Instructor response'),
  ...
  it('clicks on the button to ask to join the discussion', () => {
    ...
    expect(mockWindow.converse.askParticipantToMount).toHaveBeenCalled();
    expect(useParticipantWorkflow.getState().asked).toEqual(true);

    screen.getByText('Waiting for Instructor response');
    expect(
