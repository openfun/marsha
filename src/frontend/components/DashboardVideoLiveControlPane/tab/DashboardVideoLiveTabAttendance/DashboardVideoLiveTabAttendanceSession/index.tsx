...
const messages = defineMessages({
  connectedDiligent: {
    ...
    id: 'components.DashboardVideoLiveControlPane.tab.DashboardVideoLivettendance.DashboardVideoLivettendanceSession.connectedOver80percent',
  },
  connectedPartially: {
    defaultMessage: 'Partially present',
    description:
      'Title on icon to indicate the participant has been partially present during the live',
    id: 'components.DashboardVideoLiveControlPane.tab.DashboardVideoLivettendance.DashboardVideoLivettendanceSession.connectedPartially',
  },
  dotWasPresent: {
    defaultMessage: 'Present',
    description: 'Dot icon showing this slice of time the user was connected',
    id: 'components.DashboardVideoLiveControlPane.tab.DashboardVideoLivettendance.DashboardVideoLivettendanceSession.dotWasPresent',
  },
  dotMissed: {
    defaultMessage: 'Missed',
    description:
      'Title on dot icon showing this slice of time the user was not connected',
    id: 'components.DashboardVideoLiveControlPane.tab.DashboardVideoLivettendance.DashboardVideoLivettendanceSession.dotMissed',
  },
  notConnected: {
    defaultMessage: 'Missed the live',
    description: 'Title on icon to indicate that the user was not present',
    id: 'components.DashboardVideoLiveControlPane.tab.DashboardVideoLivettendance.DashboardVideoLivettendanceSession.notConnected',
  },
});

interface DashboardVideoLiveTabAttendanceSessionProps {
  liveSession: LiveAttendance;
}
export const DashboardVideoLiveTabAttendanceSession = ({
  liveSession,
}: DashboardVideoLiveTabAttendanceSessionProps) => {
  ...
  const getFormattedInfo = (liveAttendances: LiveAttendanceInfos) => {
    ...
    const info = Object.values(liveAttendances).map((liveAttendance) => {
      const connected = isConnected(liveAttendance);
      if (connected) nbConnected++;
      return connected;
    });
