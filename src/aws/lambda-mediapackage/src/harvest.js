...
module.exports = async (event, lambdaFunctionName) => {
  ...
  const expectFilesKey = `${elements[1]}/expected_files.json`;
  ...
  const resolutionsMap = await Promise.all(
    parsedManifest.playlists.map(async (playlist) => {
      ...
      const videoName = `${elements[2]}_${resolution}.mp4`;
      const videoKey = `${elements[1]}/mp4/${videoName}`;
      const thumbnailName = `${elements[2]}_${resolution}.0000000.jpg`;
      const thumbnailKey = `${elements[1]}/thumbnails/${thumbnailName}`;
            ...
            containerOverrides: [
              {
                ...
                environment: [
                  ...
                  {
                    name: 'VIDEO_ENDPOINT',
                    value: `${elements[1]}/video/${elements[1]}/${elements[2]}`,
                  },
                ],
              },
            ],
  ...
  const expectedFiles = resolutionsMap.reduce(
    (expectedFiles, resolutionMap) => {
      const resolution = Object.keys(resolutionMap)[0];
      expectedFiles.resolutions.push(resolution);
      expectedFiles.files.push(resolutionMap[resolution]);
