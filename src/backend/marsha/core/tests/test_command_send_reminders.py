...
class SendRemindersTest(TestCase):
    ...
    def test_send_reminders_date_has_changed(self):
        ...
        video = VideoFactory(
            ...
            starting_at_updated_on=timezone.now() - timedelta(days=200),
        )
        video_starting_at_past = VideoFactory(
            live_state=IDLE,
            live_type=RAW,
            must_notify=[settings.REMINDER_DATE_UPDATED],
            starting_at=timezone.now() + timedelta(days=200),
        ...
        video_other_tag = VideoFactory(
            live_state=IDLE,
            live_type=RAW,
            must_notify=["SOME_TAG"],
            starting_at=timezone.now() + timedelta(days=200),
            starting_at_updated_on=timezone.now() - timedelta(days=200),
        )
        video_with_other_tag = VideoFactory(
            live_state=IDLE,
            live_type=RAW,
            must_notify=["SOME_TAG", settings.REMINDER_DATE_UPDATED],
            starting_at=timezone.now() + timedelta(days=200),
            starting_at_updated_on=timezone.now() - timedelta(days=200),
        )
        ...
        # the update of starting_at is in the past
        LiveSessionFactory(
            anonymous_id=uuid.uuid4(),
            created_on=timezone.now() - timedelta(days=1),
            email="sarah@test-fun-mooc.fr",
            is_registered=True,
            should_send_reminders=True,
            video=video_starting_at_past,
        )

        # created_on is not before video updated_on
        LiveSessionFactory(
            anonymous_id=uuid.uuid4(),
            created_on=timezone.now() + timedelta(days=1),
            email="sarah_updated@test-fun-mooc.fr",
            is_registered=True,
            should_send_reminders=True,
            video=video,
        )
        ...
        lti_livesession = LiveSessionFactory(
            ...
            lti_user_id="56255f3807599c377bf0e5bf072359fd",
            should_send_reminders=True,
            video=video_with_other_tag,
        )

        # livesession with settings.REMINDER_DATE_UPDATED reminder already sent
        LiveSessionFactory(
            consumer_site=video.playlist.consumer_site,
            created_on=timezone.now() - timedelta(days=32),
        ...
        self.assertNotIn(
            settings.REMINDER_DATE_UPDATED, public_livesession.video.must_notify
        )
        self.assertNotIn(
            settings.REMINDER_DATE_UPDATED, lti_livesession.video.must_notify
        )
    ...
    def test_scenario_video_date_has_changed(self):
        ...
        starting_at = (timezone.now() + timedelta(days=300)).replace(microsecond=0)
        ...
        self.assertTrue(video.is_scheduled)
        # must_notify doesn't get updated as this video had no starting_at date before
        self.assertEqual(livesession.video.must_notify, [])
        self.assertEqual(livesession.reminders, None)
        ...
        new_starting_at = (starting_at + timedelta(days=1)).replace(microsecond=0)
        response = self.client.put(
            f"/api/videos/{video.id}/",
            {
                "title": "title",
                "starting_at": new_starting_at,
            },
            HTTP_AUTHORIZATION=f"Bearer {jwt_token}",
            content_type="application/json",
        )
        ...
        livesession_created_after = LiveSessionFactory(
            anonymous_id=uuid.uuid4(),
            email="sarah2@test-fun-mooc.fr",
            is_registered=True,
            should_send_reminders=True,
        ...
        out.close()
        livesession.refresh_from_db()
        livesession_created_after.refresh_from_db()
        self.assertEqual(livesession.video.starting_at, new_starting_at)
        # key has been added
        key_updated = (
            f"{settings.REMINDER_DATE_UPDATED}_"
            f'{dateformat.format(timezone.now(), "Y-m-d H:i")}'
        ...
        self.assertEqual(livesession_created_after.reminders, None)
        self.assertEqual(livesession.video.must_notify, [])

        # we change a new time the date
        response = self.client.put(
            f"/api/videos/{video.id}/",
            {
                "title": "title",
                "starting_at": new_starting_at + timedelta(days=100),
            },
            HTTP_AUTHORIZATION=f"Bearer {jwt_token}",
            content_type="application/json",
        )
        ...
        # if we replay the send_reminders command, now two email gets sent
        out = StringIO()
        call_command("send_reminders", stdout=out)
        self.assertIn(
            f"Sending email for livesession {livesession.id} for video "
        ...
        self.assertIn(
            f"Sending email for livesession {livesession_created_after.id} for video "
            f"{livesession_created_after.video.id} step {settings.REMINDER_DATE_UPDATED}",
            out.getvalue(),
        )
        out.close()
        ...
        livesession_created_after.refresh_from_db()
        self.assertEqual(livesession.video.must_notify, [])
        self.assertEqual(livesession_created_after.reminders, [key_updated])
