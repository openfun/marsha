"""Tests for the PortabilityRequest reject API."""
...
class PortabilityRequestAcceptAPITest(TestCase):
    """Test the "reject" API for portability request objects."""
    ...
    def assertClientPostAcceptData(
        ...
        response = self.client.post(
            f"/api/portability-requests/{portability_request_pk}/reject/",
    ...
    def test_reject_portability_request_by_anonymous_user(self):
        """Anonymous users cannot retrieve portability request."""
        portability_request = PendingPortabilityRequestFactory()

        response = self.client.post(
            f"/api/portability-requests/{portability_request.pk}/reject/"
        )
        self.assertEqual(response.status_code, 401)  # Unauthorized

    def test_reject_portability_request_by_random_logged_in_user(self):
    ...
    def test_reject_portability_request_logged_in_user_with_created_portability_request(
        self,
    ):
        """Creator of the portability request cannot reject it."""
        user = UserFactory()
    ...
    def test_reject_portability_request_logged_in_user_with_owned_playlist(self):
        """Creator of the playlist can reject the associated portability request."""
        user = UserFactory()
    ...
    def test_reject_portability_request_logged_in_user_with_playlist_admin(self):
        """Administrator of the playlist can reject the associated portability request."""
    ...
    def test_reject_portability_request_logged_in_user_with_organization_admin(self):
        """Administrator of the organization can reject the associated portability request."""
        ...
        self.assertClientPostAcceptData(
            ...
            portability_request.pk,
            {
                ...
                "id": str(portability_request.pk),
                "for_playlist": {
                    "id": str(playlist_with_organization_admin.pk),
    ...
    def test_reject_portability_request_logged_in_user_with_consumer_site_admin(self):
        """Administrator of the consumer site can reject the associated portability request."""
    ...
    def test_reject_portability_request_logged_in_user_with_playlist_admin_wrong_state(
        self,
    ):
        """
        Administrator of the playlist can't reject the associated portability request
        ...
        self.assertEqual(response.json(), {"detail": 'Method "reject" not allowed.'})
