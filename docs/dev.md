...
Unfortunately, for now, the script will generate commit messages that will not pass the gitlint check. You will have to fix them manually :

```bash
git rebase master --rebase-merges -i
```

Then, you will have to fix the commit messages manually by marking them with `reword`.
ie:
